...
<%
	...
	try {
		Site.setExcelHeader(response, out, "분기별통계-평가점수");
		...
		for(int i=1; i <= eval_order_max; i++){
			sb.append("<td class=th>"+i+"분기(총점)</td>");
			sb.append("<td class=th>"+i+"분기(평균)</td>");
		}
		...
		List<Map<String, Object>> list = db.selectList("stat_period.selectListQuarter", argMap);
